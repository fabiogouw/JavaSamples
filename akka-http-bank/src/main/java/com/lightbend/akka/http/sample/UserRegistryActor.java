...
//#user-case-classes
  ...
  public Receive createReceive(){
    return receiveBuilder()
            .match(Mensagens.GetUsers.class, getUsers -> getSender().tell(new Users(users),getSelf()))
            .match(Mensagens.CreateUser.class, createUser -> {
              users.add(createUser.getUser());
              getSender().tell(new Mensagens.ActionPerformed(
                      String.format("User %s created.", createUser.getUser().getName())),getSelf());
            })
            .match(Mensagens.GetUser.class, getUser -> {
              getSender().tell(users.stream()
                      .filter(user -> user.getName().equals(getUser.getName()))
                      .findFirst(), getSelf());
            })
            .match(Mensagens.DeleteUser.class, deleteUser -> {
              users.removeIf(user -> user.getName().equals(deleteUser.getName()));
              getSender().tell(new Mensagens.ActionPerformed(String.format("User %s deleted.", deleteUser.getName())),
                      getSelf());
